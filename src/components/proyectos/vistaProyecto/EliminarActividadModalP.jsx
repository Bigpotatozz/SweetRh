...
const EliminarActividadModalP = ({ visible3, onVisible3 }) => {
  ...
  const obtenerActividades = async () => {
    const response = await axios.get(
      `http://localhost:3000/projectActivities/`,
    );
  ...
  const eliminarActividad = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/projectActivities/delete/${actividad}`,
      );
      console.log(response);
      ...
      >
        <div className="flex gap-2 justify-between m-3">
          <Dropdown
            ...
            options={actividades}
            optionValue="id_project_activity"
            optionLabel="name"
...
export default EliminarActividadModalP;
